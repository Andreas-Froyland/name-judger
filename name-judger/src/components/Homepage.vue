<template>
  <div class="bg-base-200 w-screen h-screen">
    ...
    <div
      class="pt-24 w-5/6 center-vertical text-center text-4xl pb-6"
      v-if="showText"
    >
      <span class=""
      ...
      <span v-if="!person.age"
        >, We could not find any data about your name...</span
      >
      <span class="pb-6" v-if="person.age"
        >, You are a {{ person.age }} years old {{ person.gender }}
        <span class="pt-2" v-if="person.nation">from</span>
        {{ person.nation }}
        <span v-if="person.nationPercentage"
          >({{ person.nationPercentage }}% chance)</span
        >
      </span>
      <span class="text-xl" v-if="person.secondNation">
        or {{ person.secondNation }}({{ person.secondNationPercentage }}% chance)
      </span >
      <span class="text-sm" v-if="person.thirdNation">
        or {{ person.thirdNation }}({{ person.thirdNationPercentage }}%chance)
      </span>
    </div>
    <div class="center flex flex-col pt-20">
